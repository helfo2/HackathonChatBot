...
namespace FleetChatBotUI.ViewModels
{
    ...
    public class MainWindowViewModel : BindableBase
    {
        ...
        private string _userMessage = "Talk with Hexagon";
        /// <summary>
        /// Gets or sets the message to be sent by the user on the main textbox
        /// </summary>
        ...
        /// <summary>
        /// Gets or sets the messages exchanged between user and bot
        /// </summary>
        ...
        #region Ctors
        public MainWindowViewModel(IChatBot chatBot, IDeepSpeech sttClient)
        {
            _chatBot = chatBot;
            _sttClient = sttClient;

            var currentPath = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
            var scorerPath = Path.Combine(currentPath, InfraConfiguration.Instance().ChatBot.DeepSpeechModelRelativePath);
        ...
        public void SendMessage()
        {
            ...
            //{
            //    "What is the current state of the equipment Truck 01?",
            //    "How long is the equipment Truck 01 in the current state?",
            //    //"Who is the current operator of equipment Truck 01?"
            //};

            //foreach (var question in questions)
            //{
            //    Messages.Add(new Message { Text = question, FromUser = true, Time = DateTime.Now });
            //    var answer = Task.Run(async () => await _chatBot.Answer(question)).Result;
            //    Messages.Add(new Message { Text = answer, FromUser = false, Time = DateTime.Now });
            //}
            #endregion
            ...
            _ = Task.Run(() =>
              {
                  ...
                  {
                      Messages.Add(new Message { Text = msg, FromUser = true, Time = DateTime.Now });
                  });
              })
              .ContinueWith(t =>
              {
        ...
        /// <summary>
        /// Calls chatbot to answer to the message
        /// </summary>
        public void AnswerMessage(string msg)
        {
            string answer = Task.Run(async () => await _chatBot.Answer(msg)).Result;

            Application.Current.Dispatcher.Invoke(() => Messages.Add(new Message { Text = answer, FromUser = false, Time = DateTime.Now }));
        }
